...
curl  --limit-rate 50K  -Lbk -m 4555 -m 6555 -k   -s -d 'Username='$PATH_J_XM1'&Password='$PATH_J_XM2'&addf=2 Days Free Cccam' -X POST  https://clinepk.com/freecccam/index.php > /tmp/test/CCcam
cd /tmp/test
...
PATH_J_XM1=$(echo "$((1000 + RANDOM % 9999))$((RANDOM % 9999))")
PATH_J_XM2=$(echo "$((1000 + RANDOM % 9999))$((RANDOM % 9999))")
curl  --limit-rate 35K -Lbk -m 4555 -m 6555 -k -s -d 'Username='$PATH_J_XM1'&Password='$PATH_J_XM2'&addf=2 Days Free Cccam' -X POST  https://4gtv.us/freecccam/index.php > /tmp/test/CCcam
cd /tmp/test
echo "C: "  > hotovo
sed -ne 's#.*Server : \([^<]*\).*#\1#p' CCcam >> hotovo
echo -n " "  >> hotovo
